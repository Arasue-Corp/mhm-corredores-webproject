...
# We need to find the start of <div class="quote-container">
content_start = html.find('<div class="quote-container">')
# And the end of it (before <!-- Footer -->)
...
if content_start != -1 and footer_start != -1:
    # Before content_start, we should also update the wizard steps if they exist.
    # We can just set step 4 to completed, and add a step 5?
    # Or just keep it as step 4 but make it clear.
    # Actually, the user just wants the modal card. Let's place it inside quote-container.
    new_content = '''<div class="quote-container" style="display: flex; justify-content: center; align-items: center; min-height: 60vh;">
        <div class="harmonic-card" style="width: 100%; max-width: 400px; padding: 40px 30px; text-align: center; border-radius: 8px; box-shadow: 0 4px 6px rgba(0,0,0,0.1); position: relative;">
    ...
    # Also update cotizacion-vehicular-4.html to point to cotizacion-vehicular-5.html instead of showing the modal
    with open('cotizacion/cotizacion-vehicular-4.html', 'w', encoding='utf-8') as f:
        html4 = html.replace("document.getElementById('successModal').style.display = 'flex';", "window.location.href='cotizacion-vehicular-5.html';")
        # In case the modal was replaced inside html, let's just make sure finishFlow redirects
        f.write(html4)
    print("Updated cotizacion-vehicular-4.html to point to page 5")
