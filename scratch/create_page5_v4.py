...
# Start replacing from <div class="specs-layout-grid"> to the footer
content_start = html.find('<div class="specs-layout-grid">')
...
if content_start != -1 and footer_start != -1:
    ...
    new_content = '''<div style="display: flex; justify-content: center; align-items: center; min-height: 50vh; width: 100%; padding: 40px 20px;">
        <div class="harmonic-card" style="width: 100%; max-width: 400px; padding: 40px 30px; text-align: center; border-radius: 8px; box-shadow: 0 4px 6px rgba(0,0,0,0.1); position: relative; background: white;">
    ...
    print('Could not find markers', content_start, footer_start)

# We need to change the button on step 4 to link to step 5.
with open('cotizacion/cotizacion-vehicular-4.html', 'r', encoding='utf-8') as f:
    html4 = f.read()

# Since we previously modified successModal, let's restore it in html4 just in case,
# or simply make sure the main button goes to page 5.
html4 = re.sub(r'document\.getElementById\(\'successModal\'\)\.style\.display = \'flex\';', 'window.location.href=\'cotizacion-vehicular-5.html\';', html4)

with open('cotizacion/cotizacion-vehicular-4.html', 'w', encoding='utf-8') as f:
    f.write(html4)
print("Updated cotizacion-vehicular-4.html to link to page 5")
