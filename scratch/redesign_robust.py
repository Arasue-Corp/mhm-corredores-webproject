...
.qty-btn:hover { background: #F1F5F9 !important; transform: translateY(-1px) !important; box-shadow: 0 4px 8px rgba(0,0,0,0.06) !important; }
...
for file in files:
    ...
    # Remove the previous redesign CSS block safely if it exists
    content = re.sub(r'/\* ---- REDESIGN UX/UI LIQUID GLASS ---- \*/.*?/\* ---- END REDESIGN ---- \*/', '', content, flags=re.DOTALL)
    
    # If this robust block is already there, remove it too so we can inject a fresh one
    content = re.sub(r'/\* ---- REDESIGN UX/UI ROBUST & PREMIUM ---- \*/.*?/\* ---- END REDESIGN ROBUST ---- \*/', '', content, flags=re.DOTALL)

    # Append the robust CSS right before </style>
    content = content.replace('</style>', css_robust + '\n</style>', 1)
